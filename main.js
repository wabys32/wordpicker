...
function CreateWord(posX, posY, text, load, num21, title){
    if(load == false){
        ...
        if(canMake == true){
            ...
            localStorage.setItem(num+"title", titleField.value);
        ...
        obj.setAttribute("title", localStorage.getItem(num21+"title"));
...
function Load(){
    for(var i = 0; i < wordCount.length; i++){
        if(localStorage.getItem(i) != null){
            ...
            CreateWord(localStorage.getItem(i+"posX"), localStorage.getItem(i+"posY"), localStorage.getItem(i+"text"), true, i, localStorage.getItem(i+"title"));
        }
    }
}
    ...
    if(amountOfWords > 1){
        ...
        while(state2 == true){
            ...
            if(localStorage.getItem(t) == 1 && (t!=lastNum)){
                ...
                titleDisplay.innerHTML = localStorage.getItem(t+"title");
                wordDisplay.style.left = (window.innerWidth/2 - wordDisplay.offsetWidth/2 )+ "px";
                wordDisplay.style.top = (window.innerHeight/2 - wordDisplay.offsetHeight/2 -30)+ "px";
                titleDisplay.style.left = (window.innerWidth/2 - titleDisplay.offsetWidth/2 )+ "px";
                titleDisplay.style.top = (window.innerHeight/2 - titleDisplay.offsetHeight/2)+ "px";   
        ...
        for(var i = 0; i<wordCount.length;i++){
            if(wordCount[i] == 1){
                ...
                titleDisplay.innerHTML = localStorage.getItem(i+"title");
                wordDisplay.style.left = (window.innerWidth/2 - wordDisplay.offsetWidth/2 )+ "px";
                wordDisplay.style.top = (window.innerHeight/2 - wordDisplay.offsetHeight/2 -30)+ "px";
                titleDisplay.style.left = (window.innerWidth/2 - titleDisplay.offsetWidth/2 )+ "px";
                titleDisplay.style.top = (window.innerHeight/2 - titleDisplay.offsetHeight/2)+ "px";   
